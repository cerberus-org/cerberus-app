...
describe('siteReducer', () => {
  ...
  it('loads sites', () => {
    const result = fromSites.reducer(fromSites.initialState, new SiteActions.Load(sites)).sites;
    expect(result).toBe(sites);
  });

  it('adds a site', () => {
    const site = Object.assign({}, sites[0]);
    const result = fromSites.reducer(fromSites.initialState, new SiteActions.Add(site)).sites;
  ...
  it('modifies a site', () => {
    ...
    const result = fromSites.reducer(fromSites.initialState, new SiteActions.Modify(modified)).sites;
